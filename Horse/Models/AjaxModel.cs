...
namespace Horse.Models
{
    ...
    {
        public static Dictionary<string, string> GetAjaxParameters(HttpContextBase context)
        {
            var res = new Dictionary<string, string>();
            context.Request.InputStream.Seek(0, SeekOrigin.Begin);
            String jsonString = new StreamReader(context.Request.InputStream).ReadToEnd();
            var js = new JavaScriptSerializer();
            //jsonString = new string(jsonString.ToCharArray().Reverse().ToArray());
            res = js.Deserialize<Dictionary<string, string>>(jsonString);
            res = res.ToDictionary(kp => kp.Value, kp => kp.Key); 
            return res;
        }
